...
class CarInterface(CarInterfaceBase):
  ...
  def get_pid_accel_limits(CP, current_speed, cruise_speed):
    ...
    gas_max_bp = [0., 5.*CV.KPH_TO_MS, 10.*CV.KPH_TO_MS, 30.*CV.KPH_TO_MS, 36.*CV.KPH_TO_MS, 37.*CV.KPH_TO_MS, 48.*CV.KPH_TO_MS, 55.*CV.KPH_TO_MS, 70.*CV.KPH_TO_MS, 100.*CV.KPH_TO_MS]
    gas_max_v = [0.1, 0.4443, 0.6062, 0.5594, 0.4284, 0.3744, 0.1805, 0.13, 0.12, 0.1]

    brake_max_bp = [0., 10.*CV.KPH_TO_MS, 29.*CV.KPH_TO_MS, 30.*CV.KPH_TO_MS, 50*CV.KPH_TO_MS, 65*CV.KPH_TO_MS]
    brake_max_v = [0.28, 0.5533, 0.85201, 0.86601, 0.85601, 0.8380]
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), car_fw=[]):  # pylint: disable=dangerous-default-value
    ...
    # longitudinal
    ret.longitudinalTuning.kpBP = [0., 10. * CV.KPH_TO_MS, 20.*CV.KPH_TO_MS, 20. * CV.KPH_TO_MS, 40. * CV.KPH_TO_MS, 100. * CV.KPH_TO_MS]
